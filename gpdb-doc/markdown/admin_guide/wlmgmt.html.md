...
|Queueing|Queue only when no slot available|Queue only when no slot is available|
|Query Failure|Query may fail immediately if not enough memory|Query may fail if the allocated memory for the query surpasses the available system memory and spill limits|
